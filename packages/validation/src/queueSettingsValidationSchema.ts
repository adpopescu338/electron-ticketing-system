...
export const queueSettingsValidationSchema = Yup.object<QueueDisplaySettings>({
  maxBoxesToDisplay: Yup.number()
    .required('Max Boxes to Display is required')
  ...
  displayNumber: Yup.boolean().required('Display Number is required'),
  displayServer: Yup.boolean().required('Display Server is required'),
});
